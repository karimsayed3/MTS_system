...
createExcelForActiveSubscribers({required List<SubscriberData> data})async {
  final excel = Excel.Excel.createExcel();
  var sheet = excel['Sheet1'];
  sheet.isRTL = true;
  sheet.merge(
      Excel.CellIndex.indexByString('A1'), Excel.CellIndex.indexByString('K1'),
      customValue: Excel.TextCellValue('كشف بأسماء المشتركين'));
  ...
  header9.value = const Excel.TextCellValue("حالة الخط");
  ...
  for (var item in data) {
    sheet.appendRow(
      [
        ...
        Excel.TextCellValue(item.lineType??"لا يوجد"),
